defmodule SalxWeb.ChatRoomLive do
  use SalxWeb, :live_view

  alias Expo.Message
  alias Salx.Chat
  alias Salx.Chat.{Message, Room}
  alias Salx.Accounts.User
  alias Salx.Accounts
  alias SalxWeb.OnlineUsers
  alias SalxWeb.ChatRoomLive.ThreadComponent

  import SalxWeb.UserComponents
  import Phoenix.LiveView
  import SalxWeb.ChatComponents

  def mount(_params, _session, socket) do
    all_rooms = Chat.list_joined_rooms_with_unread_counts(socket.assigns.current_user)

    {dm_rooms, channel_rooms} =
      Enum.split_with(all_rooms, fn
        {%Salx.Chat.Room{type: "dm"}, _} -> true
        _ -> false
      end)
    ...
    users = Accounts.list_users() |> Enum.filter(&(&1.id != socket.assigns.current_user.id))
    ...
    # Subscribe doar la camerele vizibile în UI (channels)
    Enum.each(channel_rooms ++ dm_rooms, fn {chat, _} ->
      Chat.subscribe_to_room(chat)
    end)
  ...
  def handle_params(params, _uri, socket) do
    rooms = socket.assigns.all_rooms

    room =
      case Map.fetch(params, "id") do
        {:ok, id} ->
          Enum.find_value(rooms, fn
            {%Room{id: room_id} = room, _} -> if to_string(room_id) == id, do: room, else: nil
            _ -> nil
          end)
    ...
    cond do
      is_nil(room) and rooms != [] ->
        # Fallback către prima cameră validă
        [{first_room, _} | _] = rooms
        {:noreply, push_redirect(socket, to: ~p"/chat/#{first_room.id}")}

      is_nil(room) and rooms == [] ->
        {:noreply,
         socket
         |> put_flash(:error, "No chat rooms available")
         |> push_redirect(to: ~p"/rooms")}
      ...
      true ->
        ...
        socket =
          ...
          |> assign(
            ...
            room: room,
            message_cursor: page.metadata.after
          )
          |> stream(:messages, messages, reset: true)
          |> assign_message_form(Chat.change_message(%Message{}))
          |> push_event("reset_pagination", %{can_load_more: !is_nil(page.metadata.after)})
          |> push_event("scroll_messages_to_bottom", %{})
          |> update(:rooms, fn channel_rooms ->
            room_id = room.id

            Enum.map(channel_rooms, fn
              {%Room{id: ^room_id} = r, _} -> {r, 0}
              other -> other
            end)
          end)
  ...
  def render(assigns) do
    ...
    />
          ...
          <div class="relative">
            <button
              class="flex items-center peer h-8 text-sm pl-8
            pr-3 hover:bg-slate-300 cursor-pointer w-full"
            ...
            <div
              ...
              class="hidden cursor-default absolute top-8 right-2 bg-white
              border-slate-200 border py-3 rounded-lg"
              phx-click-away={JS.hide()}
            >
              <div class="w-full text-left">
                <.link
                  class="block select-none cursor-pointer whitespace-nowrap
            text-gray-800 hover:text-white px-6 py-1 block hover:bg-sky-600"
                  navigate={~p"/rooms"}
                >
                  Browse rooms
                </.link>
                <.link
                  class="block select-none cursor-pointer whitespace-nowrap
                text-gray-800 hover:text-shite px-6 py-1 block hover:bg-sky-600"
    ...
    <div class="flex flex-col grow shadow-lg">
      ...
      h-16 bg-white border-b border-slate-300 px-4">
        <div class="flex flex-col gap-1.5">
          <h1 class="text-sm font-bold leading-none">
            ...
            <.link
              :if={@joined?}
              class="font-normal text-sx text-blue-600 hover:text-blue-700"
          ...
          <%= if @current_user do %>
            <li class="text-[0.8125rem] leading-6 text-zinc-900">
              <div class="text-sm leading-10">
                ...
                >
                  <.user_avatar user={@current_user} class="h-8
        w-8 rounded" />
      ...
      >
        <%= for {dom_id, message} <- @streams.messages do %>
          <%= case message do %>
            <% :unread_marker -> %>
              <div id={dom_id} class="w-full flex text-red-500
          items-center gap-3 pr-5">
            ...
            <% %Date{} -> %>
              <div id={dom_id} class="flex flex-col items-center mt-2">
                <hr class="w-full" />
                <span class="flex items-center justify-center-mt-3 bg-white
              h-6 px-3 rounded-full border text-xs font-semibold mx-auto">
                  {format_date(message)}
                </span>
              </div>
          <% end %>
      ...
      <div :if={@joined?} class="h-12 bg-white px-4 pb-4">
        ...
        >
          <textarea
            ...
            rows="1"
          >{Phoenix.HTML.Form.normalize_value("textarea",
      @new_message_form[:body].value)}</textarea>
      ...
      <div
        :if={!@joined?}
        class="flex justify-around mx-5 pb-5 p-6 bg-slate-100 border-slate-300
        border rounded-lg"
      >
        <div class="max-w-3-xl text-center">
          <div class="mb-4">
            <h1 class="text-xl font-semibold">#{@room.name}</h1>
            <p :if={@room.topic} class="text-sm mt-1 text
    gray-600">{@room.topic}</p>
          </div>
          <div class="flex items-center justify-around">
            <button
              phx-click="join-room"
              class="px-4 py-2 bg-green-600 text-white
    rounded hover:bg-green-600 focus:outline-none focus:ring-2
    ...
    <%= if assigns[:profile] do %>
      <.live_component
        id="profile"
        module={SalxWeb.ChatRoomLive.ProfileComponent}
        user={@profile}
        current_user={@current_user}
      />
    <% end %>
    ...
    >
      ...
      <.live_component
        module={SalxWeb.ChatRoomLive.FormComponent}
  ...
  def handle_info({:deleted_reply, message}, socket) do
    socket |> refresh_message(message) |> noreply()
  end
  ...
  def handle_info({:message_deleted, message}, socket),
  ...
  def handle_event("open-dm", %{"user-id" => user_id}, socket) do
    current_user = socket.assigns.current_user
    dm_user_id = String.to_integer(user_id)

    case Chat.get_or_create_dm_room(current_user.id, dm_user_id) do
      %Room{id: room_id} ->
        {:noreply, push_redirect(socket, to: ~p"/rooms/#{room_id}")}

      nil ->
        {:noreply,
         socket
         |> put_flash(:error, "Could not open direct message channel.")
         |> push_event("notify_error", %{message: "DM room failed to load."})}
    end
  end
  ...
  defp toggler(assigns) do
    ~H"""
    <button id={@dom_id} phx-click={@on_click} class="flex
    items-center grow">
  ...
  defp unread_message_counter(assigns) do
    ...
    rounded-full font-medium h-5 px-2 ml-auto text-xs text-white"
    >
      {@count}
    </span>
    """
  ...
  defp format_date(%Date{} = date) do
    ...
    case Date.diff(today, date) do
      ...
      _ ->
        format_str =
          "%A, %B %e#{ordinal(date.day)}#{if today.year != date.year, do: " %Y", else: ""}"
  ...
  def handle_info({:new_reply, message}, socket) do
    if socket.assigns[:thread] && socket.assigns.thread.id == message.id do
      {:noreply, stream(socket, :replies, message.replies, reset: true)}
    else
      {:noreply, socket}
    end
  end
