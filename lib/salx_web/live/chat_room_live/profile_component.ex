defmodule SalxWeb.ChatRoomLive.ProfileComponent do
  ...
  def render(assigns) do
    ...
    rounded hover:bg-gray-300 ml-auto"
    ...
    text-gray-800">
             ...
             <hr class="mt-4" />
          <% else %>
            <.user_avatar user={@user} class="w-48 rounded
    mx-auto" />
          <% end %>
        </div>
  ...
  def handle_event("submit-avatar", _, socket) do
    if socket.assigns.user.id !=
         socket.assigns.current_user.id do
      raise "Prohibited"
    end
    ...
    {:ok, _user} =
      Accounts.save_user_avatar_path(
