defmodule SalxWeb.ChatRoomLive.Index do
    ...
    <main class="flex-1 p-6 max-w-4xl mx-auto">
      <div class="flex justify-between mb-4 items-center">
        ...
        <button
          ...
          class="bg-white font-semibold py-2 px-4 border
    border-slate-400 rounded shadow-sm"
        >
          Create room
        </button>
      </div>
      ...
      <div class="bg-slate-50 border rounded">
        <div id="rooms" class="divide-y" phx-update="stream">
          <div
            ...
            items-center group first:rounded-t last:rounded-b"
            id={id}
          ...
          >
            <div>
              <div class="font-medium mb-1">
                #{room.name}
                <span class="mx-1 text-gray-500 font-light
    text-sm hidden group-hover:inline group-focus:inline">
                  View room
                </span>
              </div>
            ...
            <button
              class="opacity-0 group-hover:opacity-100 group-focus:opacity-100 focus:opacity-100 bg-white hover:bg-gray-100
              border border-gray-400 text-gray-700 px-3 py-1.5 w-24 rounded_sm font-bold"
          ...
          <ul class="flex items-center -space-x-px h-10 text-base">
            <li>
              <.link
                ...
                class="flex items-center justify-center px-4 h-10 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300
  ...
  def mount(params, _session, socket) do
    ...
    # ← asigură-te că există @page din start
    |> assign(:page, 1)
    |> stream_configure(:rooms,
      dom_id: fn {room, _} -> "rooms-#{room.id}" end
    )
    |> ok()
  end
  ...
  def handle_event("toggle-room-membership", %{"id" => id}, socket) do
    ...
    socket |> stream_insert(:rooms, {room, joined?}) |> noreply()
  end
