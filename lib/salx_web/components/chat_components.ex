defmodule SalxWeb.ChatComponents do
  use SalxWeb, :html
  ...
  def message(assigns) do
    ...
    <div id={@dom_id} class="group relative flex px-4 py-3">
      <div
        ...
        class="absolute top-4 right-4 hidden group-hover:block bg-white shadow-sm px-2 pb-1 rounded border
    border-px border-slate-300 gap-1"
      >
      <button
    :if={!@in_thread?}
    phx-click={
    ...
    class="reaction-menu-button text-slate-500
      ...
      <div class="ml-2">
        <div class="-mt-1">
          ...
          >
            ...
            <a class="inline-block text-blue-600 text-xs
    font-medium ml-1" href="#">
  ...
  defp thread_avatars(assigns) do
    ...
    ~H"""
    <.user_avatar :for={user <- @users} class="h-6 w-6
    rounded shrink-0 ml-1" user={user} />
    """
  end
